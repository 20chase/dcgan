...
class DCGAN(object):
    def __init__(self, args):
        ...
        self.d2_net, self.d2_logits = self._build_discriminator(self.x_ph, is_train=True, reuse=True)
    ...
    def _build_training(self):
        grad_penalty = self._build_grad_penalty()
        with tf.variable_scope('d_loss'):
            d_loss = tf.reduce_mean(self.d_logits) - tf.reduce_mean(self.d2_logits)
            d_loss += grad_penalty

        with tf.variable_scope('g_loss'):
            g_loss = -tf.reduce_mean(self.d_logits)

        
        opter = tf.train.AdamOptimizer(learning_rate=self.args.lr, beta1=0.5, beta2=0.9)
    ...
    def _build_grad_penalty(self, scale=10.0):
        alpha = tf.random_uniform([], 0.0, 1.0)
        interpolates = alpha * self.x_ph + (1 - alpha) * self.g_net.outputs
        _, disc_interpolates = self._build_discriminator(interpolates, is_train=True, reuse=True)
        gradients = tf.gradients(disc_interpolates, [interpolates])[0]
        slopes = tf.sqrt(tf.reduce_sum(tf.square(gradients), axis=1)) 
        gradient_penalty = tf.reduce_mean((slopes-1)*scale)
    ...
    def train(self, batch_images, batch_z):
        feed_dict = {self.x_ph: batch_images,
                     self.z_ph: batch_z}
...
class DataSet(object):
    def __init__(self, args, gan):
        ...
        tl.files.exists_or_mkdir(args.sample_dir)
    ...
    def run(self):
        ...
        for e in range(self.args.epochs):
            ...
            sample = [get_image(file, 128, is_crop=self.args.is_crop, resize_w=self.args.image_size, is_grayscale=0) for file in sample_files]
            ...
            for idx in range(0, batch_idxs):
                ...
                batch = [get_image(file, 128, is_crop=self.args.is_crop, resize_w=self.args.image_size, is_grayscale=0) for file in batch_files]
